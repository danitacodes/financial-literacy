...
const Game = () => {
  ...
  return (
    <div
      name="game"
      className="flex bg-[#555B6E] pt-16 w-100 h-screen top-0 bottom-0 right-0 left-0"
    >
      <div className="flex">
        {error && <div className="fixed h-100 w-100 z-[99] bg-[#FFD6BA]"></div>}
        {success && (
          <div className="fixed h-100 w-100 z-[99] bg-lightgreen"></div>
        )}

        <div className="absolute top-20 right-10 text-4xl text-white font-Lato">
          {points}
        </div>
        <div>
          {liveItems &&
        ...
        <div className="flex self-end justify-center items-center absolute h-1/4 z-[99]">
          {buckets &&
            buckets.map((bucket, idx) => (
              <div
                key={bucket.slug}
                className={
                  "flex justify-center items-center bucket " + bucket.slug
                }
                style={{
