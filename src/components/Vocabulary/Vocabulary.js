...
const Vocabulary = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      const voices = synthRef.current.getVoices().filter(voice => !voice.name.includes('Google'));
  ...
  const choose = (choice) => {
    ...
    utter.voice = setVoice1(choice.value)
  ...
  return (
    <div name="vocab" className="bg-[#555B6E] pt-16">
      ...
      <div className="max-w-[1000px] mx-auto p-4 flex-col justify-center w-full h-full">
        <div>
          <p className='text-2xl text-[#FAF9F9] py-4 font-bold text-left'>Choose your voice</p>
          <ul className="flex w-100 my-2 mx-2">
          {voices1.map((voice) => (
            <li className='pr-2 pb-2' key={`${voice.name}`}>
              <button
                onClick={() => setVoice1(voice)}
                className={voice1 && voice1.name === voice.name ? 'selected': ''}
            ...
            {choices.map((choice) => (
              <li className="pr-2 pb-2" key={`${choice.word}-${choice.value}`}>
                <button
                  ...
                  className={!!selected[choice.value] ? 'bg-seagreen text-white opacity-50 cursor-not-allowed py-2 px-4 rounded' : 'bg-seagreen hover:bg-[#FFD6BA] text-white font-bold py-2 px-4 rounded'}
                >
                  {choice.value}
                </button>
              </li>
